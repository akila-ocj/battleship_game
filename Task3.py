...
def get_ship_configuration_count(board_size, ships, misses):
    ...
    for ship in ships:
        # If the ship has been sunk, continue to the next ship
        if ship["name"] in sunk_ships:
            continue
...
def get_highest_config_cell(board):
    ...
    return random.choice(
        highest_config_cells)  # Return a random cell if there are multiple cells with the highest count
